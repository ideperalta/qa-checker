...
async function getSitemapFromRobots(origin){
  try{var h=await fetchHtml(origin+'/robots.txt');if(!h)return null;var m=h.match(/^Sitemap:\s*(.+)$/mi);return m?m[1].trim():null;}catch(e){return null;}
}
...
async function crawlPagesBatch(urls){
  ...
  for(var i=0;i<urls.length;i+=CONCURRENCY){
    ...
    if(i+CONCURRENCY<urls.length)await new Promise(function(r){setTimeout(r,500);});
  }
...
function matchPages(bPages,cPages){
  ...
  cMap.forEach(function(cPage,path){var f=bMap.has(path)||bMap.has(path==='/'?'':path+'/')||bMap.has(path.replace(/\/$/,''));if(!f)newIn.push(cPage);});
